...
def removeOvervalues(x_data, y_data):
    # Crear una lista para almacenar los índices de las filas con valores negativos
    overRows = []
    for i, row in y_data.iterrows():
        # Revisar cada valor de la fila actual
        for val in row:
            ...
            if val > 1.85:
                ...
                break
    # Eliminar las filas con valores negativos de ambos dataframes
    x_data = x_data.drop(index=overRows)
    y_data = y_data.drop(index=overRows)
    return x_data, y_data
...
x_c_data, y_c_data, x_i_data, y_i_data = splitUtilIncompleteData(data, "Estatura")

df = saveUtilRows(data)
fig, axes = plt.subplots(nrows=1, ncols=3)

df.plot(ax=axes[0], kind='scatter', y='Estatura', x='Peso', color='red')
